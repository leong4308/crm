<x-admin::layouts>
    ...
    <x-admin::form
        :action="route('admin.settings.users.update', $admin->id)"
        method="PUT"
        ...
        <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm shadow-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                <div class="flex flex-col gap-2">
                    <x-admin::breadcrumbs name="settings.users.edit" />
            ...
            <v-users-settings-form :roles='@json($roles)' :groups='@json($groups)' :initial-user='@json($admin)'></v-users-settings-form>
        </div>
    </x-admin::form>
                ...
                <div class="flex flex-col gap-4 flex-1">
                    <div class="rounded-lg border border-gray-300 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                        ...
                        <x-admin::form.control-group v-if="!user.create_new_role">
                            <x-admin::form.control-group.label class="required">Rol Asignado</x-admin::form.control-group.label>
                            <x-admin::form.control-group.control
                                type="select"
                                name="role_id"
                                rules="required"
                        ...
                        <x-admin::form.control-group class="flex items-center gap-2.5">
                            ...
                            <label for="create_new_role" class="cursor-pointer text-sm font-semibold text-gray-600 dark:text-gray-300">¿Reemplazar por un nuevo rol personalizado?</label>
                        </x-admin::form.control-group>
                            ...
                            <div class="mt-4 space-y-4 rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
                                <div class="flex gap-4">
                                    <x-admin::form.control-group class="flex-1">
                                        ...
                                        <x-admin::form.control-group.control type="text" name="role_name" rules="required" v-model="user.role_name" />
                                    </x-admin::form.control-group>
                                    <x-admin::form.control-group class="flex-1">
                                        <x-admin::form.control-group.label class="required">Descripción</x-admin::form.control-group.label>
                                        <x-admin::form.control-group.control type="text" name="role_description" rules="required" v-model="user.role_description" />
                                    </x-admin::form.control-group>
                                </div>
                                <x-admin::form.control-group>
                        ...
                        <div class="mt-6 border-t border-gray-200 pt-6 dark:border-gray-800" v-if="user.role_id || user.create_new_role">
                            <div class="flex items-center justify-between mb-4">
                                <p class="text-sm font-bold text-gray-800 dark:text-white">Vista Previa de Permisos</p>
            ...
            app.component('v-users-settings-form', {
                ...
                data() {
                    return {
                        user: {
                            ...this.initialUser,
                            groups: this.initialUser.groups.map(g => g.id),
                ...
                mounted() {
                    // Complete previamente los permisos del rol existente, si corresponde
                    if (this.user.role_id) {
                        this.updatePermissions(this.user.role_id);
                        
                        // Expandir automáticamente al cargar
                        this.$nextTick(() => {
                            setTimeout(() => {
                ...
                watch: {
                    'user.role_id': function(newRoleId) {
                        this.updatePermissions(newRoleId);

                        // Expandir automáticamente al cambiar de rol
                        this.$nextTick(() => {
                            setTimeout(() => {
                                this.autoExpandSelected();
                            }, 500);
