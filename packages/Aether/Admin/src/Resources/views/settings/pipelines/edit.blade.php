<x-admin::layouts>
    ...
    >
        ...
        <div class="flex gap-2.5 overflow-auto py-3.5 max-xl:flex-wrap">
            <!-- Stages Component -->
            <v-pipeline-editor></v-pipeline-editor>
        </div>
    </x-admin::form>
    ...
    @pushOnce('scripts')
        ...
        <script type="module">
            app.component('v-pipeline-editor', {
                        template: '#v-pipeline-editor-template',

                        data() {
                            return {
                                stages: @json($pipeline->stages),

                                stageCount: @json(count($pipeline->stages)) + 1,
                            };
                        },
                        ...
                        methods: {
                            addStage() {
                                let position = this.stages.length >= 2 ? this.stages.length - 2 : this.stages.length;
                                
                                this.stages.splice(position, 0, {
