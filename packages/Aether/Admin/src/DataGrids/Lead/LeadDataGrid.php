    ...
    public function prepareQueryBuilder(): Builder
    {
        ...
        $queryBuilder = DB::table('leads')
            ->addSelect(
                ...
                DB::raw('MAX('.$tablePrefix.'lead_sources.name) as lead_source_name'),
                DB::raw('MAX('.$tablePrefix.'lead_types.name) as lead_type_name'),
                'leads.created_at',
                DB::raw('MAX('.$tablePrefix.'lead_pipeline_stages.name) as stage'),
                DB::raw('MAX('.$tablePrefix.'lead_tags.tag_id) as tag_id'),
                DB::raw('MAX('.$tablePrefix.'users.id) as user_id'),
                DB::raw('MAX('.$tablePrefix.'users.name) as sales_person'),
                DB::raw('MAX('.$tablePrefix.'persons.id) as person_id'),
                DB::raw('MAX('.$tablePrefix.'persons.name) as person_name'),
                DB::raw('MAX('.$tablePrefix.'tags.name) as tag_name'),
                DB::raw('MAX('.$tablePrefix.'lead_pipelines.rotten_days) as pipeline_rotten_days'),
                DB::raw('MAX('.$tablePrefix.'lead_pipeline_stages.code) as stage_code'),
                DB::raw('CASE WHEN EXTRACT(EPOCH FROM (NOW() - '.$tablePrefix.'leads.created_at)) / 86400 >= MAX('.$tablePrefix.'lead_pipelines.rotten_days) THEN 1 ELSE 0 END as rotten_lead'),
            )
        ...
        $this->addFilter('rotten_lead', DB::raw('EXTRACT(EPOCH FROM (NOW() - '.$tablePrefix.'leads.created_at)) / 86400 >= '.$tablePrefix.'lead_pipelines.rotten_days'));
